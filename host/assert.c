...
void __assert_fail_i32(int32_t assert_num, int32_t expected, int32_t actual) {
  fprintf(stderr,
          "Assertion failure in assert_eq expression %d: expected %d, got %d\n",
...
void __assert_fail_i64(int32_t assert_num, int64_t expected, int64_t actual) {
  fprintf(stderr,
          "Assertion failure in assert_eq expression %d: expected %" PRId64
          ", got %" PRId64 "\n",
          assert_num, expected, actual);
  exit_status = 1;
}
...
void __assert_fail_f32(int32_t assert_num, float expected, float actual) {
  fprintf(stderr,
          "Assertion failure in assert_eq expression %d: expected %f, got %f\n",
          assert_num, expected, actual);
  exit_status = 1;
}
...
void __assert_fail_f64(int32_t assert_num, double expected, double actual) {
  fprintf(stderr,
          "Assertion failure in assert_eq expression %d: expected %a, got %a",
          assert_num, expected, actual);
  exit_status = 1;
}
